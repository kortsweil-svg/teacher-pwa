...
<div id="app-screen" style="display:none">
  <header>
    ...
    <nav>
      ...
      <button class="nav-btn" onclick="showScreen('history')">היסטוריה</button>
  ...
  <main>
    ...
    <div id="screen-dashboard" class="screen active">
      ...
      <div style="display:flex;gap:8px;align-items:center;margin-bottom:1rem;flex-wrap:wrap">
        <label style="font-size:13px;color:var(--text2)">מ:</label>
        <input type="date" id="dash-from" style="padding:6px 10px;border:0.5px solid var(--border2);border-radius:var(--r);background:var(--bg);color:var(--text);font-size:13px" onchange="renderDashboard()"/>
        <label style="font-size:13px;color:var(--text2)">עד:</label>
        <input type="date" id="dash-to" style="padding:6px 10px;border:0.5px solid var(--border2);border-radius:var(--r);background:var(--bg);color:var(--text);font-size:13px" onchange="renderDashboard()"/>
        <button class="btn" onclick="setDashRange('today')">היום</button>
        <button class="btn" onclick="setDashRange('7d')">שבוע</button>
        <button class="btn" onclick="setDashRange('30d')">חודש</button>
      </div>
      <!-- קוד מוסד בולט -->
    ...
    <div id="screen-students" class="screen">
      <div style="display:flex;gap:8px;align-items:center;margin-bottom:1rem;flex-wrap:wrap">
        <label style="font-size:13px;color:var(--text2)">מ:</label>
        <input type="date" id="students-from" style="padding:6px 10px;border:0.5px solid var(--border2);border-radius:var(--r);background:var(--bg);color:var(--text);font-size:13px" onchange="renderStudents()"/>
        <label style="font-size:13px;color:var(--text2)">עד:</label>
        <input type="date" id="students-to" style="padding:6px 10px;border:0.5px solid var(--border2);border-radius:var(--r);background:var(--bg);color:var(--text);font-size:13px" onchange="renderStudents()"/>
        <button class="btn" onclick="setStudentsRange('today')">היום</button>
        <button class="btn" onclick="setStudentsRange('7d')">שבוע</button>
        <button class="btn" onclick="setStudentsRange('30d')">חודש</button>
        <button class="btn" onclick="setStudentsRange('')">כל הזמן</button>
      </div>
      <div class="card">
        <div class="toolbar">
    ...
    <div id="screen-history" class="screen">
      <div class="card">
        <div class="toolbar">
          <div class="card-title" style="margin-bottom:0">היסטוריית זמן מסך</div>
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            <select id="hist-range" onchange="renderHistory()">
              <option value="today">היום</option>
              <option value="yesterday">אתמול</option>
              <option value="7d" selected>שבוע אחרון</option>
              ...
              <option value="all">מאז ההתקנה</option>
            </select>
            <select id="hist-class" onchange="renderHistory()"><option value="">כל הכיתות</option></select>
          </div>
        </div>
        <div id="history-chart" style="margin-top:1rem"></div>
...
function setDashRange(r){
  ...
  if(!from||!to) return;
  to.value = getToday();
  if(r==='today') from.value = getToday();
  else if(r==='7d') from.value = getDaysAgo(7);
  else if(r==='30d') from.value = getDaysAgo(30);
  renderDashboard();
}

function setStudentsRange(r){
  const from = document.getElementById('students-from');
  const to = document.getElementById('students-to');
  if(!from||!to) return;
  to.value = getToday();
  if(r==='today') from.value = getToday();
  else if(r==='7d') from.value = getDaysAgo(7);
...
async function renderDashboard(){
  ...
  let data;
  if(from && to) {
    data = await api('/api/students/range?from='+from+'&to='+to);
  } else {
    data = await api('/api/students');
  }
...
async function renderStudents(){
  ...
  const from = document.getElementById('students-from')?.value;
  const to = document.getElementById('students-to')?.value;
  let data;
  if(from && to) {
    data = await api('/api/students/range?from='+from+'&to='+to);
  } else {
      ...
      <div style="display:flex;align-items:center;gap:12px">
        ...
        <div style="flex:1;min-width:0">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
            ...
            <span style="font-size:11px;color:var(--text2);margin-right:6px">${s.className}</span>
            ${s.mood ? '<span style="font-size:16px;margin-right:4px">'+(s.mood==='good'?'😊':s.mood==='neutral'?'😐':'😟')+'</span>' : ''}</div>
...
async function renderHistory(){
  const range = document.getElementById('hist-range')?.value || '7d';
  const cls = document.getElementById('hist-class')?.value || '';
  const data = await api('/api/history?range='+range);
  if(data.error) return;
  const rows = data.rows || [];

  // עדכן כיתות
  const classes = [...new Set(rows.map(s=>s.class_name).filter(Boolean))].sort();
  const sel = document.getElementById('hist-class');
  if(sel){const cur=sel.value;sel.innerHTML='<option value="">כל הכיתות</option>'+classes.map(c=>`<option value="${c}">${c}</option>`).join('');sel.value=cur;}

  // סנן לפי כיתה
  const filtered = cls ? rows.filter(s=>s.class_name===cls) : rows;

  if(!filtered.filter(r=>r.report_date).length){
    document.getElementById('history-chart').innerHTML='';
    document.getElementById('history-list').innerHTML='<div style="color:var(--text2);font-size:13px;padding:1rem 0">אין נתונים עדיין — נתונים יצטברו עם הסנכרונים הבאים</div>';
    return;
  }
  ...
  const dates_sorted = Object.keys(byDate).sort().reverse();

  // גרף יומי
  const dateAvgs = dates_sorted.map(d => {
    const r = byDate[d].filter(s=>s.daily_average>0);
    const avg = r.length ? r.reduce((a,s)=>a+parseFloat(s.daily_average),0)/r.length : 0;
    return {date:d, avg};
  }).reverse().slice(-14);

  const maxAvg = Math.max(...dateAvgs.map(w=>w.avg), 1);
  document.getElementById('history-chart').innerHTML = dateAvgs.length > 1 ? `
    <div style="font-size:12px;color:var(--text2);margin-bottom:6px">ממוצע יומי</div>
    <div style="display:flex;align-items:flex-end;gap:4px;height:80px">
      ${dateAvgs.map(d=>`
        <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:2px">
          <div style="font-size:9px;color:var(--text2)">${d.avg.toFixed(1)}</div>
          <div style="width:100%;border-radius:3px 3px 0 0;background:${color(d.avg)};height:${Math.max(2,Math.round(d.avg/maxAvg*60))}px"></div>
          <div style="font-size:9px;color:var(--text2)">${d.date.slice(5)}</div>
        </div>`).join('')}
    </div>` : '';

  // טבלה לפי תאריך
  document.getElementById('history-list').innerHTML = dates_sorted.map(d => {
    const dayRows = byDate[d];
    const avg = dayRows.filter(r=>r.daily_average>0).reduce((a,r,i,arr)=>a+parseFloat(r.daily_average)/arr.length,0);
    return `
      <div style="margin-bottom:.75rem">
        <div style="font-size:13px;font-weight:500;color:var(--text);margin-bottom:4px;display:flex;justify-content:space-between">
          <span>${d}</span>
          <span style="font-size:12px;color:var(--text2)">ממוצע: ${avg.toFixed(1)}ש'</span>
        </div>
        ${dayRows.filter(r=>r.report_date).map(r=>`
          <div class="student-row" style="padding:5px 0">
            <div class="avatar" style="width:26px;height:26px;font-size:10px">${r.name?.slice(0,2)}</div>
            <div style="flex:1;font-size:13px">${r.name}</div>
            <div style="font-size:12px;color:var(--text2)">${r.class_name}</div>
            <div style="font-size:13px;font-weight:500">${r.daily_average>0?parseFloat(r.daily_average).toFixed(1)+"ש'":'—'}</div>
          </div>`).join('')}
      </div>`;
  }).join('');
}
